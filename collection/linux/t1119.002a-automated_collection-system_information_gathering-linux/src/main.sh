    ...
    export T1119_002A_DEBUG_MODE="${T1119_002A_DEBUG_MODE:-false}"
    export T1119_002A_TIMEOUT="${T1119_002A_TIMEOUT:-300}"
    export T1119_002A_FALLBACK_MODE="${T1119_002A_FALLBACK_MODE:-real}"
    export T1119_002A_OUTPUT_FORMAT="${T1119_002A_OUTPUT_FORMAT:-json}"
    export T1119_002A_POLICY_CHECK="${T1119_002A_POLICY_CHECK:-true}"
    export T1119_002A_MAX_FILES="${T1119_002A_MAX_FILES:-200}"
    export T1119_002A_MAX_FILE_SIZE="${T1119_002A_MAX_FILE_SIZE:-1048576}"
    export T1119_002A_SCAN_DEPTH="${T1119_002A_SCAN_DEPTH:-3}"
    export T1119_002A_EXCLUDE_CACHE="${T1119_002A_EXCLUDE_CACHE:-true}"
    export T1119_002A_CAPTURE_DURATION="${T1119_002A_CAPTURE_DURATION:-60}"
    # ===== FIN VARIABLES COLLECTION =====

#!/bin/bash

# T1119.002a - Automated Collection: System Information Gathering Linux
# MITRE ATT&CK Enterprise - Collection Tactic (TA0009)
# ATOMIC ACTION: Automatically gather system information ONLY
...
Check-CriticalDeps() { 
    for cmd in bash jq bc grep find stat ps; do 
        command -v "$cmd" >/dev/null || { 
            [[ "${T1119_002A_OUTPUT_MODE:-simple}" != "stealth" ]] && echo "[ERROR] Missing dependency: $cmd"; exit 1; 
        }
    done
}
...
Load-EnvironmentVariables() {
    export T1119_002A_OUTPUT_BASE="${T1119_002A_OUTPUT_BASE:-./mitre_results}"
    export T1119_002A_TIMEOUT="${T1119_002A_TIMEOUT:-300}"
    export T1119_002A_OUTPUT_MODE="${T1119_002A_OUTPUT_MODE:-simple}"
    export T1119_002A_SILENT_MODE="${T1119_002A_SILENT_MODE:-false}"
    export T1119_002A_MAX_INFO_FILES="${T1119_002A_MAX_INFO_FILES:-50}"
    
    export T1119_002A_INFO_CATEGORIES="${T1119_002A_INFO_CATEGORIES:-system,network,processes,users,hardware}"
    export T1119_002A_INCLUDE_HARDWARE="${T1119_002A_INCLUDE_HARDWARE:-true}"
    export T1119_002A_INCLUDE_NETWORK="${T1119_002A_INCLUDE_NETWORK:-true}"
    export T1119_002A_INCLUDE_PROCESSES="${T1119_002A_INCLUDE_PROCESSES:-true}"
    export T1119_002A_INCLUDE_USERS="${T1119_002A_INCLUDE_USERS:-true}"
}

# System preconditions validation
Validate-SystemPreconditions() {
    [[ -z "$T1119_002A_OUTPUT_BASE" ]] && { [[ "${T1119_002A_OUTPUT_MODE:-simple}" != "stealth" ]] && echo "[ERROR] T1119_002A_OUTPUT_BASE not set"; return 1; }
    [[ ! -w "$(dirname "$T1119_002A_OUTPUT_BASE")" ]] && { [[ "${T1119_002A_OUTPUT_MODE:-simple}" != "stealth" ]] && echo "[ERROR] Output directory not writable"; return 1; }
    return 0
}
...
Initialize-OutputStructure() {
    ...
    export COLLECTION_DIR="$T1119_002A_OUTPUT_BASE/T1119_002a_system_info_$timestamp"
    mkdir -p "$COLLECTION_DIR"/{system_info,metadata} 2>/dev/null || return 1
...
Gather-SystemInformation() {
    local collection_dir="$1" category="$2"
    
    local info_file="$collection_dir/system_info/${category}_info_$(date +%s).txt"
    ...
    if [[ -f "$info_file" && -s "$info_file" ]]; then
        ...
        [[ "$T1119_002A_SILENT_MODE" != "true" ]] && echo "  + Gathered: $category info ($file_size bytes)" >&2
        return 0
    fi
    return 1
}
...
Log-ExecutionMessage() {
    local message="$1"
    # Silent in stealth mode or when T1119_002A_SILENT_MODE is true
    [[ "$T1119_002A_SILENT_MODE" != "true" && "${T1119_002A_OUTPUT_MODE:-simple}" != "stealth" ]] && echo "$message" >&2
}

# Simple output generation
Generate-SimpleOutput() {
    local files_collected="$1" total_size="$2" collection_dir="$3"
    echo "SYSTEM INFO GATHERING "
...
EOF
    ...
    [[ "$T1119_002A_SILENT_MODE" != "true" ]] && echo "$json_output"
}
...
Invoke-MicroTechniqueAction() {
    ...
    Log-ExecutionMessage "[INFO] Gathering system information..."
    
    # ATOMIC ACTION: Orchestration of auxiliary functions
    IFS=',' read -ra categories <<< "$T1119_002A_INFO_CATEGORIES"
    
    for category in "${categories[@]}"; do
        category=$(echo "$category" | xargs)
        
        # Check if category is enabled
        local var_name="T1119_002A_INCLUDE_$(echo "$category" | tr '[:lower:]' '[:upper:]')"
        local include_category="${!var_name:-true}"
        
        if [[ "$include_category" == "true" ]]; then
            if result=$(Gather-SystemInformation "$collection_dir" "$category"); then
                IFS=':' read -r file_path file_size <<< "$result"
                collected_files+=("$file_path")
                total_size=$((total_size + file_size))
                ((file_count++))
                [[ $file_count -ge ${T1119_002A_MAX_INFO_FILES:-50} ]] && break
            fi
        fi
    done
