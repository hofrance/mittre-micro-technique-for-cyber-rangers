...
# DESCRIPTION: Découverte des comptes domaine
...
function Get-Configuration {
    echo "[ATTACK] Loading configuration for real discovery attack: T1087.002a" >&2
    ...
    export TT1087_002A_BACKUP_ORIGINAL="false"
    export TT1087_002A_AUTO_ROLLBACK="false"

    # BASIC ATOMIC VARIABLES
    export TT1087_002A_DEBUG_MODE="${TT1087_002A_DEBUG_MODE:-false}"
    export TT1087_002A_OUTPUT_MODE="${TT1087_002A_OUTPUT_MODE:-simple}"
    export TT1087_002A_VERBOSE_LEVEL="${TT1087_002A_VERBOSE_LEVEL:-1}"
    export TT1087_002A_AUDIT_LOGGING="${TT1087_002A_AUDIT_LOGGING:-true}"

    # OUTPUT ATOMIC VARIABLES
    export TT1087_002A_OUTPUT_BASE="/tmp/mitre_results"
    export TT1087_002A_OUTPUT_FORMAT="json"
    ...
    export TT1087_002A_RESULTS_DIR="${TT1087_002A_OUTPUT_BASE}/T1087_002a_${timestamp}"

    mkdir -p "${TT1087_002A_RESULTS_DIR}" 2>/dev/null || {
    ...
    export T1087_002A_DEBUG_MODE="${T1087_002A_DEBUG_MODE:-false}"
    export T1087_002A_TIMEOUT="${T1087_002A_TIMEOUT:-300}"
    export T1087_002A_FALLBACK_MODE="${T1087_002A_FALLBACK_MODE:-simulate}"
    export T1087_002A_OUTPUT_FORMAT="${T1087_002A_OUTPUT_FORMAT:-json}"
    export T1087_002A_POLICY_CHECK="${T1087_002A_POLICY_CHECK:-true}"
    export T1087_002A_MAX_SERVICES="${T1087_002A_MAX_SERVICES:-200}"
    export T1087_002A_INCLUDE_SYSTEM="${T1087_002A_INCLUDE_SYSTEM:-true}"
    export T1087_002A_DETAIL_LEVEL="${T1087_002A_DETAIL_LEVEL:-standard}"
    export T1087_002A_RESOLVE_HOSTNAMES="${T1087_002A_RESOLVE_HOSTNAMES:-true}"
    export T1087_002A_MAX_PROCESSES="${T1087_002A_MAX_PROCESSES:-500}"
...
function Precondition-Check {
    echo "[ATTACK] Performing minimal precondition check for T1087.002a" >&2
...
function Atomic-Action {
    echo "[ATTACK] Executing real atomic discovery: Domain Account Discovery" >&2

    local results_file="${TT1087_002A_RESULTS_DIR}/discovery_results.json"
    local discovery_start=$(date +%s)

    # REAL DISCOVERY EXECUTION - NO SIMULATION, NO SAFEGUARDS
    echo "T1087.002a REAL DISCOVERY EXECUTION ==="
    echo "Starting real discovery execution at $(date)"
    echo "Technique: T1087.002a"
    echo "Action: Domain Account Discovery"
    echo "Description: Découverte des comptes domaine"
    echo "Training: Elite Cyber Guardians"
    echo "==="

    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    T1087_002a_real_discovery_commands
    ...
    cat > "$results_file" << JSON_EOF
{
  ...
  "mitre.technique.name": "account_discovery-domain_account",
  "discovery": {
  ...
  "attack": {
    "technique": "T1087.002a",
    "action": "Domain Account Discovery",
    "description": "Découverte des comptes domaine",
    "safeguards": "removed",
    "operational": true,
...
JSON_EOF
    ...
    echo "[ATTACK] Real discovery completed - Results: ${TT1087_002A_RESULTS_DIR}" >&2
...
function Write-StandardizedOutput {
    ...
    case "${TT1087_002A_OUTPUT_MODE}" in
        "simple")
            echo "T1087.002a - account_discovery-domain_account"
            echo "Atomic Action: Domain Account Discovery"
            echo "Result: INFORMATION_GATHERED"
            echo "Training: Elite Cyber Guardians"
            echo "Results: ${TT1087_002A_RESULTS_DIR}"
            ;;
        "debug")
            if [[ -f "$results_file" ]]; then
            ...
            ;;
        "stealth")
            echo "[T1087.002a] Discovery executed"
            ;;
        *)
            echo "[ATTACK] Real discovery completed - Results: ${TT1087_002A_RESULTS_DIR}" >&2
            ;;
...
function Main {
    echo " Starting real discovery execution for T1087.002a" >&2
    ...
    echo " Real discovery execution completed for T1087.002a" >&2
    return 0  # SUCCESS
}
# REAL DISCOVERY COMMANDS DEFINITION

function T1087_002a_real_discovery_commands {
    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    case "T1087.002a" in
        ...
        *)
            echo "[REAL DISCOVERY] Generic discovery technique T1087.002a"
