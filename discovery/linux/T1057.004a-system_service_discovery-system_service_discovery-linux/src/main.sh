...
# T1057.004a - system_service_discovery-system_service_discovery
# DISCOVERY -  OPERATIONAL TRAINING
# ATOMIC ACTION: System Service Discovery
# DESCRIPTION: Découverte des services système
...
function Get-Configuration {
    echo "[ATTACK] Loading configuration for real discovery attack: T1057.004a" >&2

    # DIRECT ATOMIC VARIABLES - NO SAFEGUARDS
    export TT1057_004A_SIMULATION_MODE="false"
    export TT1057_004A_SAFETY_CHECKS="false"
    export TT1057_004A_REQUIRE_CONFIRMATION="false"
    export TT1057_004A_BACKUP_ORIGINAL="false"
    export TT1057_004A_AUTO_ROLLBACK="false"

    # BASIC ATOMIC VARIABLES
    export TT1057_004A_DEBUG_MODE="${TT1057_004A_DEBUG_MODE:-false}"
    export TT1057_004A_OUTPUT_MODE="${TT1057_004A_OUTPUT_MODE:-simple}"
    export TT1057_004A_VERBOSE_LEVEL="${TT1057_004A_VERBOSE_LEVEL:-1}"
    export TT1057_004A_AUDIT_LOGGING="${TT1057_004A_AUDIT_LOGGING:-true}"

    # OUTPUT ATOMIC VARIABLES
    export TT1057_004A_OUTPUT_BASE="/tmp/mitre_results"
    export TT1057_004A_OUTPUT_FORMAT="json"
    ...
    export TT1057_004A_RESULTS_DIR="${TT1057_004A_OUTPUT_BASE}/T1057_004a_${timestamp}"

    mkdir -p "${TT1057_004A_RESULTS_DIR}" 2>/dev/null || {
    ...
    export T1057_004A_DEBUG_MODE="${T1057_004A_DEBUG_MODE:-false}"
    export T1057_004A_TIMEOUT="${T1057_004A_TIMEOUT:-300}"
    export T1057_004A_FALLBACK_MODE="${T1057_004A_FALLBACK_MODE:-simulate}"
    export T1057_004A_OUTPUT_FORMAT="${T1057_004A_OUTPUT_FORMAT:-json}"
    export T1057_004A_POLICY_CHECK="${T1057_004A_POLICY_CHECK:-true}"
    export T1057_004A_MAX_SERVICES="${T1057_004A_MAX_SERVICES:-200}"
    export T1057_004A_INCLUDE_SYSTEM="${T1057_004A_INCLUDE_SYSTEM:-true}"
    export T1057_004A_DETAIL_LEVEL="${T1057_004A_DETAIL_LEVEL:-standard}"
    export T1057_004A_RESOLVE_HOSTNAMES="${T1057_004A_RESOLVE_HOSTNAMES:-true}"
    export T1057_004A_MAX_PROCESSES="${T1057_004A_MAX_PROCESSES:-500}"
...
function Precondition-Check {
    echo "[ATTACK] Performing minimal precondition check for T1057.004a" >&2
...
function Atomic-Action {
    echo "[ATTACK] Executing real atomic discovery: System Service Discovery" >&2

    local results_file="${TT1057_004A_RESULTS_DIR}/discovery_results.json"
    local discovery_start=$(date +%s)

    # REAL DISCOVERY EXECUTION - NO SIMULATION, NO SAFEGUARDS
    echo "T1057.004a REAL DISCOVERY EXECUTION ==="
    echo "Starting real discovery execution at $(date)"
    echo "Technique: T1057.004a"
    echo "Action: System Service Discovery"
    echo "Description: Découverte des services système"
    echo "Training: Elite Cyber Guardians"
    echo "==="

    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    T1057_004a_real_discovery_commands
    ...
    cat > "$results_file" << JSON_EOF
{
  ...
  "mitre.technique.id": "T1057.004a",
  "mitre.technique.name": "system_service_discovery-system_service_discovery",
  "discovery": {
  ...
  "attack": {
    "technique": "T1057.004a",
    "action": "System Service Discovery",
    "description": "Découverte des services système",
    "safeguards": "removed",
    "operational": true,
...
JSON_EOF
    ...
    echo "[ATTACK] Real discovery completed - Results: ${TT1057_004A_RESULTS_DIR}" >&2
...
function Write-StandardizedOutput {
    ...
    case "${TT1057_004A_OUTPUT_MODE}" in
        "simple")
            echo "T1057.004a - system_service_discovery-system_service_discovery"
            echo "Atomic Action: System Service Discovery"
            echo "Result: INFORMATION_GATHERED"
            echo "Training: Elite Cyber Guardians"
            echo "Results: ${TT1057_004A_RESULTS_DIR}"
            ;;
        "debug")
            if [[ -f "$results_file" ]]; then
            ...
            ;;
        "stealth")
            echo "[T1057.004a] Discovery executed"
            ;;
        *)
            echo "[ATTACK] Real discovery completed - Results: ${TT1057_004A_RESULTS_DIR}" >&2
            ;;
...
function Main {
    echo " Starting real discovery execution for T1057.004a" >&2
    ...
    echo " Real discovery execution completed for T1057.004a" >&2
    return 0  # SUCCESS
}
# REAL DISCOVERY COMMANDS DEFINITION

function T1057_004a_real_discovery_commands {
    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    case "T1057.004a" in
        ...
        *)
            echo "[REAL DISCOVERY] Generic discovery technique T1057.004a"
