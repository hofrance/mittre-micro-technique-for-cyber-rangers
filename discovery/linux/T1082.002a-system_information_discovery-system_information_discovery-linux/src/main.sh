...
# T1082.002a - system_information_discovery-system_information_discovery
# DISCOVERY -  OPERATIONAL TRAINING
# ATOMIC ACTION: System Information Discovery
# DESCRIPTION: Découverte des informations système générales
...
function Get-Configuration {
    echo "[ATTACK] Loading configuration for real discovery attack: T1082.002a" >&2

    # DIRECT ATOMIC VARIABLES - NO SAFEGUARDS
    export TT1082_002A_SIMULATION_MODE="false"
    export TT1082_002A_SAFETY_CHECKS="false"
    export TT1082_002A_REQUIRE_CONFIRMATION="false"
    export TT1082_002A_BACKUP_ORIGINAL="false"
    export TT1082_002A_AUTO_ROLLBACK="false"

    # BASIC ATOMIC VARIABLES
    export TT1082_002A_DEBUG_MODE="${TT1082_002A_DEBUG_MODE:-false}"
    export TT1082_002A_OUTPUT_MODE="${TT1082_002A_OUTPUT_MODE:-simple}"
    export TT1082_002A_VERBOSE_LEVEL="${TT1082_002A_VERBOSE_LEVEL:-1}"
    export TT1082_002A_AUDIT_LOGGING="${TT1082_002A_AUDIT_LOGGING:-true}"

    # OUTPUT ATOMIC VARIABLES
    export TT1082_002A_OUTPUT_BASE="/tmp/mitre_results"
    export TT1082_002A_OUTPUT_FORMAT="json"
    ...
    export TT1082_002A_RESULTS_DIR="${TT1082_002A_OUTPUT_BASE}/T1082_002a_${timestamp}"

    mkdir -p "${TT1082_002A_RESULTS_DIR}" 2>/dev/null || {
    ...
    export T1082_002A_DEBUG_MODE="${T1082_002A_DEBUG_MODE:-false}"
    export T1082_002A_TIMEOUT="${T1082_002A_TIMEOUT:-300}"
    export T1082_002A_FALLBACK_MODE="${T1082_002A_FALLBACK_MODE:-simulate}"
    export T1082_002A_OUTPUT_FORMAT="${T1082_002A_OUTPUT_FORMAT:-json}"
    export T1082_002A_POLICY_CHECK="${T1082_002A_POLICY_CHECK:-true}"
    export T1082_002A_MAX_SERVICES="${T1082_002A_MAX_SERVICES:-200}"
    export T1082_002A_INCLUDE_SYSTEM="${T1082_002A_INCLUDE_SYSTEM:-true}"
    export T1082_002A_DETAIL_LEVEL="${T1082_002A_DETAIL_LEVEL:-standard}"
    export T1082_002A_RESOLVE_HOSTNAMES="${T1082_002A_RESOLVE_HOSTNAMES:-true}"
    export T1082_002A_MAX_PROCESSES="${T1082_002A_MAX_PROCESSES:-500}"
...
function Precondition-Check {
    echo "[ATTACK] Performing minimal precondition check for T1082.002a" >&2
...
function Atomic-Action {
    echo "[ATTACK] Executing real atomic discovery: System Information Discovery" >&2

    local results_file="${TT1082_002A_RESULTS_DIR}/discovery_results.json"
    local discovery_start=$(date +%s)

    # REAL DISCOVERY EXECUTION - NO SIMULATION, NO SAFEGUARDS
    echo "T1082.002a REAL DISCOVERY EXECUTION ==="
    echo "Starting real discovery execution at $(date)"
    echo "Technique: T1082.002a"
    echo "Action: System Information Discovery"
    echo "Description: Découverte des informations système générales"
    echo "Training: Elite Cyber Guardians"
    echo "==="

    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    T1082_002a_real_discovery_commands
    ...
    cat > "$results_file" << JSON_EOF
{
  ...
  "mitre.technique.id": "T1082.002a",
  "mitre.technique.name": "system_information_discovery-system_information_discovery",
  "discovery": {
  ...
  "attack": {
    "technique": "T1082.002a",
    "action": "System Information Discovery",
    "description": "Découverte des informations système générales",
    "safeguards": "removed",
    "operational": true,
...
JSON_EOF
    ...
    echo "[ATTACK] Real discovery completed - Results: ${TT1082_002A_RESULTS_DIR}" >&2
...
function Write-StandardizedOutput {
    ...
    case "${TT1082_002A_OUTPUT_MODE}" in
        "simple")
            echo "T1082.002a - system_information_discovery-system_information_discovery"
            echo "Atomic Action: System Information Discovery"
            echo "Result: INFORMATION_GATHERED"
            echo "Training: Elite Cyber Guardians"
            echo "Results: ${TT1082_002A_RESULTS_DIR}"
            ;;
        "debug")
            if [[ -f "$results_file" ]]; then
            ...
            ;;
        "stealth")
            echo "[T1082.002a] Discovery executed"
            ;;
        *)
            echo "[ATTACK] Real discovery completed - Results: ${TT1082_002A_RESULTS_DIR}" >&2
            ;;
...
function Main {
    echo " Starting real discovery execution for T1082.002a" >&2
    ...
    echo " Real discovery execution completed for T1082.002a" >&2
    return 0  # SUCCESS
}
# REAL DISCOVERY COMMANDS DEFINITION

function T1082_002a_real_discovery_commands {
    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    case "T1082.002a" in
        ...
        *)
            echo "[REAL DISCOVERY] Generic discovery technique T1082.002a"
