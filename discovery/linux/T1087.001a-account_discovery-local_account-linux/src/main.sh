...
# DESCRIPTION: Découverte des comptes locaux
...
function Get-Configuration {
    echo "[ATTACK] Loading configuration for real discovery attack: T1087.001a" >&2
    ...
    export TT1087_001A_BACKUP_ORIGINAL="false"
    export TT1087_001A_AUTO_ROLLBACK="false"

    # BASIC ATOMIC VARIABLES
    export TT1087_001A_DEBUG_MODE="${TT1087_001A_DEBUG_MODE:-false}"
    export TT1087_001A_OUTPUT_MODE="${TT1087_001A_OUTPUT_MODE:-simple}"
    export TT1087_001A_VERBOSE_LEVEL="${TT1087_001A_VERBOSE_LEVEL:-1}"
    export TT1087_001A_AUDIT_LOGGING="${TT1087_001A_AUDIT_LOGGING:-true}"

    # OUTPUT ATOMIC VARIABLES
    export TT1087_001A_OUTPUT_BASE="/tmp/mitre_results"
    export TT1087_001A_OUTPUT_FORMAT="json"
    ...
    export TT1087_001A_RESULTS_DIR="${TT1087_001A_OUTPUT_BASE}/T1087_001a_${timestamp}"

    mkdir -p "${TT1087_001A_RESULTS_DIR}" 2>/dev/null || {
...
function Precondition-Check {
    echo "[ATTACK] Performing minimal precondition check for T1087.001a" >&2
...
function Atomic-Action {
    echo "[ATTACK] Executing real atomic discovery: Local Account Discovery" >&2

    local results_file="${TT1087_001A_RESULTS_DIR}/discovery_results.json"
    local discovery_start=$(date +%s)

    # REAL DISCOVERY EXECUTION - NO SIMULATION, NO SAFEGUARDS
    echo "T1087.001a REAL DISCOVERY EXECUTION ==="
    echo "Starting real discovery execution at $(date)"
    echo "Technique: T1087.001a"
    echo "Action: Local Account Discovery"
    echo "Description: Découverte des comptes locaux"
    echo "Training: Elite Cyber Guardians"
    echo "==="

    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    T1087_001a_real_discovery_commands
    ...
    cat > "$results_file" << JSON_EOF
{
  ...
  "mitre.technique.name": "account_discovery-local_account",
  "discovery": {
  ...
  "attack": {
    "technique": "T1087.001a",
    "action": "Local Account Discovery",
    "description": "Découverte des comptes locaux",
    "safeguards": "removed",
    "operational": true,
...
JSON_EOF
    ...
    echo "[ATTACK] Real discovery completed - Results: ${TT1087_001A_RESULTS_DIR}" >&2
...
function Write-StandardizedOutput {
    ...
    case "${TT1087_001A_OUTPUT_MODE}" in
        "simple")
            echo "T1087.001a - account_discovery-local_account"
            echo "Atomic Action: Local Account Discovery"
            echo "Result: INFORMATION_GATHERED"
            echo "Training: Elite Cyber Guardians"
            echo "Results: ${TT1087_001A_RESULTS_DIR}"
            ;;
        "debug")
            if [[ -f "$results_file" ]]; then
            ...
            ;;
        "stealth")
            echo "[T1087.001a] Discovery executed"
            ;;
        *)
            echo "[ATTACK] Real discovery completed - Results: ${TT1087_001A_RESULTS_DIR}" >&2
            ;;
...
function Main {
    echo " Starting real discovery execution for T1087.001a" >&2
    ...
    echo " Real discovery execution completed for T1087.001a" >&2
    return 0  # SUCCESS
}
# REAL DISCOVERY COMMANDS DEFINITION

function T1087_001a_real_discovery_commands {
    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    case "T1087.001a" in
        ...
        *)
            echo "[REAL DISCOVERY] Generic discovery technique T1087.001a"
