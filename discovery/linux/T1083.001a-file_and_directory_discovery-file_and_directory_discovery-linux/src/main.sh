...
# T1083.001a - file_and_directory_discovery-file_and_directory_discovery
# DISCOVERY -  OPERATIONAL TRAINING
# ATOMIC ACTION: File and Directory Discovery
# DESCRIPTION: Découverte de fichiers et répertoires
...
function Get-Configuration {
    echo "[ATTACK] Loading configuration for real discovery attack: T1083.001a" >&2

    # DIRECT ATOMIC VARIABLES - NO SAFEGUARDS
    export TT1083_001A_SIMULATION_MODE="false"
    export TT1083_001A_SAFETY_CHECKS="false"
    export TT1083_001A_REQUIRE_CONFIRMATION="false"
    export TT1083_001A_BACKUP_ORIGINAL="false"
    export TT1083_001A_AUTO_ROLLBACK="false"

    # BASIC ATOMIC VARIABLES
    export TT1083_001A_DEBUG_MODE="${TT1083_001A_DEBUG_MODE:-false}"
    export TT1083_001A_OUTPUT_MODE="${TT1083_001A_OUTPUT_MODE:-simple}"
    export TT1083_001A_VERBOSE_LEVEL="${TT1083_001A_VERBOSE_LEVEL:-1}"
    export TT1083_001A_AUDIT_LOGGING="${TT1083_001A_AUDIT_LOGGING:-true}"

    # OUTPUT ATOMIC VARIABLES
    export TT1083_001A_OUTPUT_BASE="/tmp/mitre_results"
    export TT1083_001A_OUTPUT_FORMAT="json"
    ...
    export TT1083_001A_RESULTS_DIR="${TT1083_001A_OUTPUT_BASE}/T1083_001a_${timestamp}"

    mkdir -p "${TT1083_001A_RESULTS_DIR}" 2>/dev/null || {
...
function Precondition-Check {
    echo "[ATTACK] Performing minimal precondition check for T1083.001a" >&2
...
function Atomic-Action {
    echo "[ATTACK] Executing real atomic discovery: File and Directory Discovery" >&2

    local results_file="${TT1083_001A_RESULTS_DIR}/discovery_results.json"
    local discovery_start=$(date +%s)

    # REAL DISCOVERY EXECUTION - NO SIMULATION, NO SAFEGUARDS
    echo "T1083.001a REAL DISCOVERY EXECUTION ==="
    echo "Starting real discovery execution at $(date)"
    echo "Technique: T1083.001a"
    echo "Action: File and Directory Discovery"
    echo "Description: Découverte de fichiers et répertoires"
    echo "Training: Elite Cyber Guardians"
    echo "==="

    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    T1083_001a_real_discovery_commands
    ...
    cat > "$results_file" << JSON_EOF
{
  ...
  "mitre.technique.id": "T1083.001a",
  "mitre.technique.name": "file_and_directory_discovery-file_and_directory_discovery",
  "discovery": {
  ...
  "attack": {
    ...
    "action": "File and Directory Discovery",
    "description": "Découverte de fichiers et répertoires",
    "safeguards": "removed",
    "operational": true,
...
JSON_EOF
    ...
    echo "[ATTACK] Real discovery completed - Results: ${TT1083_001A_RESULTS_DIR}" >&2
...
function Write-StandardizedOutput {
    ...
    case "${TT1083_001A_OUTPUT_MODE}" in
        "simple")
            echo "T1083.001a - file_and_directory_discovery-file_and_directory_discovery"
            echo "Atomic Action: File and Directory Discovery"
            echo "Result: INFORMATION_GATHERED"
            echo "Training: Elite Cyber Guardians"
            echo "Results: ${TT1083_001A_RESULTS_DIR}"
            ;;
        "debug")
            if [[ -f "$results_file" ]]; then
            ...
            ;;
        "stealth")
            echo "[T1083.001a] Discovery executed"
            ;;
        *)
            echo "[ATTACK] Real discovery completed - Results: ${TT1083_001A_RESULTS_DIR}" >&2
            ;;
...
function Main {
    echo " Starting real discovery execution for T1083.001a" >&2
    ...
    echo " Real discovery execution completed for T1083.001a" >&2
    return 0  # SUCCESS
}
# REAL DISCOVERY COMMANDS DEFINITION

function T1083_001a_real_discovery_commands {
    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    case "T1083.001a" in
        ...
        *)
            echo "[REAL DISCOVERY] Generic discovery technique T1083.001a"
