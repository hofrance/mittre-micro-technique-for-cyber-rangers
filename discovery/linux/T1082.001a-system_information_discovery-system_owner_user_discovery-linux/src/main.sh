...
# T1082.001a - system_information_discovery-system_owner_user_discovery
# DISCOVERY -  OPERATIONAL TRAINING
# ATOMIC ACTION: System Owner/User Discovery
# DESCRIPTION: Découverte des propriétaires et utilisateurs système
...
function Get-Configuration {
    echo "[ATTACK] Loading configuration for real discovery attack: T1082.001a" >&2

    # DIRECT ATOMIC VARIABLES - NO SAFEGUARDS
    export TT1082_001A_SIMULATION_MODE="false"
    export TT1082_001A_SAFETY_CHECKS="false"
    export TT1082_001A_REQUIRE_CONFIRMATION="false"
    export TT1082_001A_BACKUP_ORIGINAL="false"
    export TT1082_001A_AUTO_ROLLBACK="false"

    # BASIC ATOMIC VARIABLES
    export TT1082_001A_DEBUG_MODE="${TT1082_001A_DEBUG_MODE:-false}"
    export TT1082_001A_OUTPUT_MODE="${TT1082_001A_OUTPUT_MODE:-simple}"
    export TT1082_001A_VERBOSE_LEVEL="${TT1082_001A_VERBOSE_LEVEL:-1}"
    export TT1082_001A_AUDIT_LOGGING="${TT1082_001A_AUDIT_LOGGING:-true}"

    # OUTPUT ATOMIC VARIABLES
    export TT1082_001A_OUTPUT_BASE="/tmp/mitre_results"
    export TT1082_001A_OUTPUT_FORMAT="json"
    ...
    export TT1082_001A_RESULTS_DIR="${TT1082_001A_OUTPUT_BASE}/T1082_001a_${timestamp}"

    mkdir -p "${TT1082_001A_RESULTS_DIR}" 2>/dev/null || {
...
function Precondition-Check {
    echo "[ATTACK] Performing minimal precondition check for T1082.001a" >&2
...
function Atomic-Action {
    echo "[ATTACK] Executing real atomic discovery: System Owner/User Discovery" >&2

    local results_file="${TT1082_001A_RESULTS_DIR}/discovery_results.json"
    local discovery_start=$(date +%s)

    # REAL DISCOVERY EXECUTION - NO SIMULATION, NO SAFEGUARDS
    echo "T1082.001a REAL DISCOVERY EXECUTION ==="
    echo "Starting real discovery execution at $(date)"
    echo "Technique: T1082.001a"
    echo "Action: System Owner/User Discovery"
    echo "Description: Découverte des propriétaires et utilisateurs système"
    echo "Training: Elite Cyber Guardians"
    echo "==="

    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    T1082_001a_real_discovery_commands
    ...
    cat > "$results_file" << JSON_EOF
{
  ...
  "mitre.technique.id": "T1082.001a",
  "mitre.technique.name": "system_information_discovery-system_owner_user_discovery",
  "discovery": {
  ...
  "attack": {
    ...
    "action": "System Owner/User Discovery",
    "description": "Découverte des propriétaires et utilisateurs système",
    "safeguards": "removed",
    "operational": true,
...
JSON_EOF
    ...
    echo "[ATTACK] Real discovery completed - Results: ${TT1082_001A_RESULTS_DIR}" >&2
...
function Write-StandardizedOutput {
    ...
    case "${TT1082_001A_OUTPUT_MODE}" in
        "simple")
            echo "T1082.001a - system_information_discovery-system_owner_user_discovery"
            echo "Atomic Action: System Owner/User Discovery"
            echo "Result: INFORMATION_GATHERED"
            echo "Training: Elite Cyber Guardians"
            echo "Results: ${TT1082_001A_RESULTS_DIR}"
            ;;
        "debug")
            if [[ -f "$results_file" ]]; then
            ...
            ;;
        "stealth")
            echo "[T1082.001a] Discovery executed"
            ;;
        *)
            echo "[ATTACK] Real discovery completed - Results: ${TT1082_001A_RESULTS_DIR}" >&2
            ;;
...
function Main {
    echo " Starting real discovery execution for T1082.001a" >&2
    ...
    echo " Real discovery execution completed for T1082.001a" >&2
    return 0  # SUCCESS
}
# REAL DISCOVERY COMMANDS DEFINITION

function T1082_001a_real_discovery_commands {
    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    case "T1082.001a" in
        ...
        *)
            echo "[REAL DISCOVERY] Generic discovery technique T1082.001a"
