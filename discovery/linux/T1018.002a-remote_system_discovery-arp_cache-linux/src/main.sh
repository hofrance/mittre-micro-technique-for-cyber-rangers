...
# T1018.002a - remote_system_discovery-arp_cache
# DISCOVERY -  OPERATIONAL TRAINING
# ATOMIC ACTION: ARP Cache Discovery
# DESCRIPTION: Découverte du cache ARP
...
function Get-Configuration {
    echo "[ATTACK] Loading configuration for real discovery attack: T1018.002a" >&2

    # DIRECT ATOMIC VARIABLES - NO SAFEGUARDS
    export TT1018_002A_SIMULATION_MODE="false"
    export TT1018_002A_SAFETY_CHECKS="false"
    export TT1018_002A_REQUIRE_CONFIRMATION="false"
    export TT1018_002A_BACKUP_ORIGINAL="false"
    export TT1018_002A_AUTO_ROLLBACK="false"

    # BASIC ATOMIC VARIABLES
    export TT1018_002A_DEBUG_MODE="${TT1018_002A_DEBUG_MODE:-false}"
    export TT1018_002A_OUTPUT_MODE="${TT1018_002A_OUTPUT_MODE:-simple}"
    export TT1018_002A_VERBOSE_LEVEL="${TT1018_002A_VERBOSE_LEVEL:-1}"
    export TT1018_002A_AUDIT_LOGGING="${TT1018_002A_AUDIT_LOGGING:-true}"

    # OUTPUT ATOMIC VARIABLES
    export TT1018_002A_OUTPUT_BASE="/tmp/mitre_results"
    export TT1018_002A_OUTPUT_FORMAT="json"
    ...
    export TT1018_002A_RESULTS_DIR="${TT1018_002A_OUTPUT_BASE}/T1018_002a_${timestamp}"

    mkdir -p "${TT1018_002A_RESULTS_DIR}" 2>/dev/null || {
    ...
    export T1018_002A_DEBUG_MODE="${T1018_002A_DEBUG_MODE:-false}"
    export T1018_002A_TIMEOUT="${T1018_002A_TIMEOUT:-300}"
    export T1018_002A_FALLBACK_MODE="${T1018_002A_FALLBACK_MODE:-simulate}"
    export T1018_002A_OUTPUT_FORMAT="${T1018_002A_OUTPUT_FORMAT:-json}"
    export T1018_002A_POLICY_CHECK="${T1018_002A_POLICY_CHECK:-true}"
    export T1018_002A_MAX_SERVICES="${T1018_002A_MAX_SERVICES:-200}"
    export T1018_002A_INCLUDE_SYSTEM="${T1018_002A_INCLUDE_SYSTEM:-true}"
    export T1018_002A_DETAIL_LEVEL="${T1018_002A_DETAIL_LEVEL:-standard}"
    export T1018_002A_RESOLVE_HOSTNAMES="${T1018_002A_RESOLVE_HOSTNAMES:-true}"
    export T1018_002A_MAX_PROCESSES="${T1018_002A_MAX_PROCESSES:-500}"
...
function Precondition-Check {
    echo "[ATTACK] Performing minimal precondition check for T1018.002a" >&2
...
function Atomic-Action {
    echo "[ATTACK] Executing real atomic discovery: ARP Cache Discovery" >&2

    local results_file="${TT1018_002A_RESULTS_DIR}/discovery_results.json"
    local discovery_start=$(date +%s)

    # REAL DISCOVERY EXECUTION - NO SIMULATION, NO SAFEGUARDS
    echo "T1018.002a REAL DISCOVERY EXECUTION ==="
    echo "Starting real discovery execution at $(date)"
    echo "Technique: T1018.002a"
    echo "Action: ARP Cache Discovery"
    echo "Description: Découverte du cache ARP"
    echo "Training: Elite Cyber Guardians"
    echo "==="

    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    T1018_002a_real_discovery_commands
    ...
    cat > "$results_file" << JSON_EOF
{
  ...
  "mitre.technique.id": "T1018.002a",
  "mitre.technique.name": "remote_system_discovery-arp_cache",
  "discovery": {
  ...
  "attack": {
    "technique": "T1018.002a",
    "action": "ARP Cache Discovery",
    "description": "Découverte du cache ARP",
    "safeguards": "removed",
    "operational": true,
...
JSON_EOF
    ...
    echo "[ATTACK] Real discovery completed - Results: ${TT1018_002A_RESULTS_DIR}" >&2
...
function Write-StandardizedOutput {
    ...
    case "${TT1018_002A_OUTPUT_MODE}" in
        "simple")
            echo "T1018.002a - remote_system_discovery-arp_cache"
            echo "Atomic Action: ARP Cache Discovery"
            echo "Result: INFORMATION_GATHERED"
            echo "Training: Elite Cyber Guardians"
            echo "Results: ${TT1018_002A_RESULTS_DIR}"
            ;;
        "debug")
            if [[ -f "$results_file" ]]; then
            ...
            ;;
        "stealth")
            echo "[T1018.002a] Discovery executed"
            ;;
        *)
            echo "[ATTACK] Real discovery completed - Results: ${TT1018_002A_RESULTS_DIR}" >&2
            ;;
...
function Main {
    echo " Starting real discovery execution for T1018.002a" >&2
    ...
    echo " Real discovery execution completed for T1018.002a" >&2
    return 0  # SUCCESS
}
# REAL DISCOVERY COMMANDS DEFINITION

function T1018_002a_real_discovery_commands {
    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    case "T1018.002a" in
        ...
        *)
            echo "[REAL DISCOVERY] Generic discovery technique T1018.002a"
