...
# DESCRIPTION: Découverte des périphériques
...
function Get-Configuration {
    echo "[ATTACK] Loading configuration for real discovery attack: T1083.005a" >&2
    ...
    export TT1083_005A_BACKUP_ORIGINAL="false"
    export TT1083_005A_AUTO_ROLLBACK="false"

    # BASIC ATOMIC VARIABLES
    export TT1083_005A_DEBUG_MODE="${TT1083_005A_DEBUG_MODE:-false}"
    export TT1083_005A_OUTPUT_MODE="${TT1083_005A_OUTPUT_MODE:-simple}"
    export TT1083_005A_VERBOSE_LEVEL="${TT1083_005A_VERBOSE_LEVEL:-1}"
    export TT1083_005A_AUDIT_LOGGING="${TT1083_005A_AUDIT_LOGGING:-true}"

    # OUTPUT ATOMIC VARIABLES
    export TT1083_005A_OUTPUT_BASE="/tmp/mitre_results"
    export TT1083_005A_OUTPUT_FORMAT="json"
    ...
    export TT1083_005A_RESULTS_DIR="${TT1083_005A_OUTPUT_BASE}/T1083_005a_${timestamp}"

    mkdir -p "${TT1083_005A_RESULTS_DIR}" 2>/dev/null || {
    ...
    export T1083_005A_DEBUG_MODE="${T1083_005A_DEBUG_MODE:-false}"
    export T1083_005A_TIMEOUT="${T1083_005A_TIMEOUT:-300}"
    export T1083_005A_FALLBACK_MODE="${T1083_005A_FALLBACK_MODE:-simulate}"
    export T1083_005A_OUTPUT_FORMAT="${T1083_005A_OUTPUT_FORMAT:-json}"
    export T1083_005A_POLICY_CHECK="${T1083_005A_POLICY_CHECK:-true}"
    export T1083_005A_MAX_SERVICES="${T1083_005A_MAX_SERVICES:-200}"
    export T1083_005A_INCLUDE_SYSTEM="${T1083_005A_INCLUDE_SYSTEM:-true}"
    export T1083_005A_DETAIL_LEVEL="${T1083_005A_DETAIL_LEVEL:-standard}"
    export T1083_005A_RESOLVE_HOSTNAMES="${T1083_005A_RESOLVE_HOSTNAMES:-true}"
    export T1083_005A_MAX_PROCESSES="${T1083_005A_MAX_PROCESSES:-500}"
...
function Precondition-Check {
    echo "[ATTACK] Performing minimal precondition check for T1083.005a" >&2
...
function Atomic-Action {
    echo "[ATTACK] Executing real atomic discovery: Peripheral Device Discovery" >&2

    local results_file="${TT1083_005A_RESULTS_DIR}/discovery_results.json"
    local discovery_start=$(date +%s)

    # REAL DISCOVERY EXECUTION - NO SIMULATION, NO SAFEGUARDS
    echo "T1083.005a REAL DISCOVERY EXECUTION ==="
    echo "Starting real discovery execution at $(date)"
    echo "Technique: T1083.005a"
    echo "Action: Peripheral Device Discovery"
    echo "Description: Découverte des périphériques"
    echo "Training: Elite Cyber Guardians"
    echo "==="

    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    T1083_005a_real_discovery_commands
    ...
    cat > "$results_file" << JSON_EOF
{
  ...
  "mitre.technique.name": "file_and_directory_discovery-peripheral_device_discovery",
  "discovery": {
  ...
  "attack": {
    "technique": "T1083.005a",
    "action": "Peripheral Device Discovery",
    "description": "Découverte des périphériques",
    "safeguards": "removed",
    "operational": true,
...
JSON_EOF
    ...
    echo "[ATTACK] Real discovery completed - Results: ${TT1083_005A_RESULTS_DIR}" >&2
...
function Write-StandardizedOutput {
    ...
    case "${TT1083_005A_OUTPUT_MODE}" in
        "simple")
            echo "T1083.005a - file_and_directory_discovery-peripheral_device_discovery"
            echo "Atomic Action: Peripheral Device Discovery"
            echo "Result: INFORMATION_GATHERED"
            echo "Training: Elite Cyber Guardians"
            echo "Results: ${TT1083_005A_RESULTS_DIR}"
            ;;
        "debug")
            if [[ -f "$results_file" ]]; then
            ...
            ;;
        "stealth")
            echo "[T1083.005a] Discovery executed"
            ;;
        *)
            echo "[ATTACK] Real discovery completed - Results: ${TT1083_005A_RESULTS_DIR}" >&2
            ;;
...
function Main {
    echo " Starting real discovery execution for T1083.005a" >&2
    ...
    echo " Real discovery execution completed for T1083.005a" >&2
    return 0  # SUCCESS
}
# REAL DISCOVERY COMMANDS DEFINITION

function T1083_005a_real_discovery_commands {
    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    case "T1083.005a" in
        ...
        *)
            echo "[REAL DISCOVERY] Generic discovery technique T1083.005a"
