...
# T1057.001a - process_discovery-process_discovery
# DISCOVERY -  OPERATIONAL TRAINING
# ATOMIC ACTION: Process Discovery
# DESCRIPTION: Découverte des processus en cours
...
function Get-Configuration {
    echo "[ATTACK] Loading configuration for real discovery attack: T1057.001a" >&2

    # DIRECT ATOMIC VARIABLES - NO SAFEGUARDS
    export TT1057_001A_SIMULATION_MODE="false"
    export TT1057_001A_SAFETY_CHECKS="false"
    export TT1057_001A_REQUIRE_CONFIRMATION="false"
    export TT1057_001A_BACKUP_ORIGINAL="false"
    export TT1057_001A_AUTO_ROLLBACK="false"

    # BASIC ATOMIC VARIABLES
    export TT1057_001A_DEBUG_MODE="${TT1057_001A_DEBUG_MODE:-false}"
    export TT1057_001A_OUTPUT_MODE="${TT1057_001A_OUTPUT_MODE:-simple}"
    export TT1057_001A_VERBOSE_LEVEL="${TT1057_001A_VERBOSE_LEVEL:-1}"
    export TT1057_001A_AUDIT_LOGGING="${TT1057_001A_AUDIT_LOGGING:-true}"

    # OUTPUT ATOMIC VARIABLES
    export TT1057_001A_OUTPUT_BASE="/tmp/mitre_results"
    export TT1057_001A_OUTPUT_FORMAT="json"
    ...
    export TT1057_001A_RESULTS_DIR="${TT1057_001A_OUTPUT_BASE}/T1057_001a_${timestamp}"

    mkdir -p "${TT1057_001A_RESULTS_DIR}" 2>/dev/null || {
...
function Precondition-Check {
    echo "[ATTACK] Performing minimal precondition check for T1057.001a" >&2
...
function Atomic-Action {
    echo "[ATTACK] Executing real atomic discovery: Process Discovery" >&2

    local results_file="${TT1057_001A_RESULTS_DIR}/discovery_results.json"
    local discovery_start=$(date +%s)

    # REAL DISCOVERY EXECUTION - NO SIMULATION, NO SAFEGUARDS
    echo "T1057.001a REAL DISCOVERY EXECUTION ==="
    echo "Starting real discovery execution at $(date)"
    echo "Technique: T1057.001a"
    echo "Action: Process Discovery"
    echo "Description: Découverte des processus en cours"
    echo "Training: Elite Cyber Guardians"
    echo "==="

    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    T1057_001a_real_discovery_commands
    ...
    cat > "$results_file" << JSON_EOF
{
  ...
  "mitre.technique.id": "T1057.001a",
  "mitre.technique.name": "process_discovery-process_discovery",
  "discovery": {
  ...
  "attack": {
    "technique": "T1057.001a",
    "action": "Process Discovery",
    "description": "Découverte des processus en cours",
    "safeguards": "removed",
    "operational": true,
...
JSON_EOF
    ...
    echo "[ATTACK] Real discovery completed - Results: ${TT1057_001A_RESULTS_DIR}" >&2
...
function Write-StandardizedOutput {
    ...
    case "${TT1057_001A_OUTPUT_MODE}" in
        "simple")
            echo "T1057.001a - process_discovery-process_discovery"
            echo "Atomic Action: Process Discovery"
            echo "Result: INFORMATION_GATHERED"
            echo "Training: Elite Cyber Guardians"
            echo "Results: ${TT1057_001A_RESULTS_DIR}"
            ;;
        "debug")
            if [[ -f "$results_file" ]]; then
            ...
            ;;
        "stealth")
            echo "[T1057.001a] Discovery executed"
            ;;
        *)
            echo "[ATTACK] Real discovery completed - Results: ${TT1057_001A_RESULTS_DIR}" >&2
            ;;
...
function Main {
    echo " Starting real discovery execution for T1057.001a" >&2
    ...
    echo " Real discovery execution completed for T1057.001a" >&2
    return 0  # SUCCESS
}
# REAL DISCOVERY COMMANDS DEFINITION

function T1057_001a_real_discovery_commands {
    # REAL DISCOVERY COMMANDS - ACTUAL SYSTEM INFORMATION GATHERING
    case "T1057.001a" in
        ...
        *)
            echo "[REAL DISCOVERY] Generic discovery technique T1057.001a"
