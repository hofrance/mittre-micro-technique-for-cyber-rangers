...
# ATOMIC ACTION: Discover running processes
# Platform: Windows
...
function Invoke-MicroTechniqueAction {
    ...
    try {
        Write-Host "   Discovering running processes..." -ForegroundColor Cyan

        $processes = Get-Process | Where-Object { $_.Id -ne $PID } | Select-Object Name, Id
        ...
        return @{ Success = $true; Data = "Process discovery completed" }
...
function Write-StandardizedOutput {
    ...
    if (-not $Config.SilentMode) {
        ...
        if ($Config.Results.Success) {
            Write-Host "   Status: Process discovery completed"
        } else {
